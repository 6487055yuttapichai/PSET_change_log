...
CREATE OR REPLACE FUNCTION dbo."Report_Trace_Curve"(
	in_ids text[])
    RETURNS TABLE(device_id integer, tightening_id integer, mid text, "timestamp" timestamp without time zone, trace_data text) 
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
    RETURN QUERY SELECT DISTINCT
		...
		tc.mid::TEXT,
		(tc."stringifiedDataJSON"::JSON -> 'payload' ->> 'timeStamp')::TIMESTAMP WITHOUT TIME ZONE AS "timestamp",
