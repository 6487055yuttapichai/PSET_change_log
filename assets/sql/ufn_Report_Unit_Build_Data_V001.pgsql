-- FUNCTION: dbo.Report_Unit_Build_Data_V002(timestamp without time zone, timestamp without time zone, character varying, character varying, character varying)

-- DROP FUNCTION IF EXISTS dbo."Report_Unit_Build_Data_V002"(timestamp without time zone, timestamp without time zone, character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION dbo."Report_Unit_Build_Data_V002"(
	in_from_date timestamp without time zone,
	in_to_date timestamp without time zone,
	in_station character varying DEFAULT '%'::character varying,
	in_model character varying DEFAULT '%'::character varying,
	in_serial character varying DEFAULT '%'::character varying)
...
BEGIN
    ...
    WHERE (t."updatedAt" >= in_from_date AND t."updatedAt" < in_to_date)
...
ALTER FUNCTION dbo."Report_Unit_Build_Data_V002"(timestamp without time zone, timestamp without time zone, character varying, character varying, character varying)
    OWNER TO postgres;
